// Package config defines the typed configuration for the Room Service.
// Room is a producer (outbox + relay), so its config includes Kafka producer
// settings alongside the standard DB/Redis/Web/Log/OTel. Mirrors the auth-
...
// Config is the full Room Service configuration.
type Config struct {
...
// RedisConfig is the Redis connection (for relay leader election).
...
// Defaults returns the Viper defaults map for the Room Service.
