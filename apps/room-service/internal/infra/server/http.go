// Package server wires the Room Service to the HTTP transport.
package server
...
func New(opts Options) (http.Handler, *gin.Engine) {
	...
	connectPath, connectHandler := roomv1connect.NewRoomServiceHandler(
		opts.Service,
		vbrpc.New(vbrpc.Options{ServiceName: "room-service"})...,
	)
