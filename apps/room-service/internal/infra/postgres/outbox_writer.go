...
// OutboxWriter implements ports.OutboxWriter. Mirrors the auth-service pattern.
type OutboxWriter struct{}

// NewOutboxWriter constructs an OutboxWriter.
func NewOutboxWriter() *OutboxWriter { return &OutboxWriter{} }

// Append inserts one or more outbox events within the given transaction.
func (OutboxWriter) Append(ctx context.Context, tx pgx.Tx, events ...vboutbox.Event) error {
	for i, e := range events {
		...
		// Payload is already the serialized event body ([]byte) — insert it
		// verbatim. Re-marshaling a []byte would base64-encode it into a JSON
		// string and break every consumer's json.Unmarshal.
		payload := e.Payload
...
// PendingEvent is an unpublished outbox row fetched by the relay.
