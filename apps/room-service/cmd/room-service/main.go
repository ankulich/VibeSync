// Command room-service is the VibeSync Room Service entrypoint.
//
// Wire order (producer pattern, like Auth):
...
import (
	...
	"vibesync/apps/room-service/internal/app"
	"vibesync/apps/room-service/internal/config"
	"vibesync/apps/room-service/internal/infra/events"
	"vibesync/apps/room-service/internal/infra/migrate"
	"vibesync/apps/room-service/internal/infra/postgres"
	vbredis "vibesync/apps/room-service/internal/infra/redis"
	"vibesync/apps/room-service/internal/infra/server"
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintf(os.Stderr, "room-service: fatal: %v\n", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	vbweb.SetReady(true)
	logger.Info("room-service ready")
