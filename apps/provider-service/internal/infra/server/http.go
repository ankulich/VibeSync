// Package server wires the Provider Service to the HTTP transport.
package server
...
func New(opts Options) (http.Handler, *gin.Engine) {
	...
	connectPath, connectHandler := providerv1connect.NewProviderServiceHandler(
		opts.Service,
		vbrpc.New(vbrpc.Options{ServiceName: "provider-service"})...,
	)
